      ...
      <div className="mx-auto max-w-6xl px-6 py-12">
        <section className="text-center pt-8">
          ...
          <p className="mt-4 mx-auto max-w-2xl text-gray-500">
            The complete Web4 ecosystem on NEAR Protocol with integrated AI. 
            Create, connect with others, and start exploring, building, and earning.
          </p>
          ...
          <div className="mt-8 items-center justify-center gap-4">
            <div className="flex items-center justify-center gap-4">
              {!isConnected ? (
                <button 
                  ...
                  onClick={async () => { 
                    try {
                      ...
                      // Ask header to open the wallet panel so this button mirrors the top-right behavior
                ...
                <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('nearcity-open-wallet'))} className="hidden md:inline-flex items-center gap-3 px-4 py-2 rounded-full bg-white border">
                  <div className="w-1 h-6 bg-teal-600 rounded" />
                  <span className="text-sm text-gray-700">{walletName ? walletName.split('.')[0] : 'Wallet'}</span>
                </button>
              )}

              <button type="button" onClick={() => setMobileMenuOpen(true)} className="md:hidden inline-flex items-center gap-2 px-3 py-2 rounded-full border">
          ...
          <div className="mt-14 grid grid-cols-1 sm:grid-cols-3 gap-6 items-stretch">
            <article className="bg-blue-100/40 rounded-3xl p-8 shadow-md sm:transform sm:-rotate-3">
            ...
            <article className="bg-red-100/40 rounded-3xl p-8 shadow-md sm:transform sm:rotate-3">
